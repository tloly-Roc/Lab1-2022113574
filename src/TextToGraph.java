...
public class TextToGraph {
        ...
        while (true) {
            ...
            switch (choice) {
                ...
                case 5:
                    ...
                    Map<String, Double> pageRank = graphBuilder.calculatePageRank();
                    System.out.println("PageRank结果:");
     ...
     * @param dampingFactor Typically 0.85 (probability to follow links vs random jump)
     * @param maxIterations Maximum number of iterations
     * @param tolerance Convergence threshold
     * @return Map of node to its PageRank score
     */
    public Map<String, Double> calculatePageRank(double dampingFactor, int maxIterations, double tolerance) {
        ...
        Map<String, Double> prevRank = new HashMap<>();
        Map<String, Double> currRank = new HashMap<>();

        // Initialize all nodes with equal rank
        for (String node : directedGraph.keySet()) {
            prevRank.put(node, initialScore);
        }
        ...
        for (int i = 0; i < maxIterations; i++) {
            ...
            // Calculate sum of dangling node ranks (nodes with no outgoing edges)
            for (String node : directedGraph.keySet()) {
                if (directedGraph.get(node).isEmpty()) {
                    danglingSum += prevRank.get(node);
                }
            }
            ...
            for (String node : directedGraph.keySet()) {
                ...
                // Sum over all nodes that link to this node
                for (Map.Entry<String, Map<String, Integer>> entry : directedGraph.entrySet()) {
                    ...
                    if (edges.containsKey(node)) {
                        ...
                        sum += prevRank.get(source) / outDegree;
                    }
                }

                // Apply PageRank formula with dangling node contribution
                double newRank = (1 - dampingFactor) / numNodes +
                        dampingFactor * (sum + (danglingSum / numNodes));
                currRank.put(node, newRank);
            }

            // Check for convergence
            if (hasConverged(prevRank, currRank, tolerance)) {
                System.out.println("PageRank converged after " + (i+1) + " iterations");
                break;
            }

            // Prepare for next iteration
            prevRank = new HashMap<>(currRank);
            currRank.clear();
        }

        // Normalize the final ranks (so they sum to 1)
        normalizeRanks(prevRank);

        // Store the results
        pageRankScores.clear();
        pageRankScores.putAll(prevRank);

        return prevRank;
    }

    /**
     * Simplified PageRank calculation with default parameters
     */
    public Map<String, Double> calculatePageRank() {
        return calculatePageRank(0.85, 100, 0.0001);
    }

    /**
     * Checks if PageRank has converged
     */
    private boolean hasConverged(Map<String, Double> prev, Map<String, Double> curr, double tolerance) {
        double totalDiff = 0.0;
        for (String node : prev.keySet()) {
            totalDiff += Math.abs(prev.get(node) - curr.get(node));
        }
